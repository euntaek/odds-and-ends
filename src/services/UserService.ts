...
class UserService {
  //FIXME: 포스트서비스에도 있음. 후에 확인 후 없애던지 하자.
  /* 
  ...
  */
  ...
  // FIXME:
  // * 사용자 찾기와 가져오기를 하나로 통합하고 컨트롤러 부분에서 이리 저리 사용하는 걸로 생각해보자. 중복이라 보기 안좋음.
  // * 변수명도 좀 수정하자.
  // * 보니까 여러 사용자의 정보를 가져오는 메소드가 없다. 것도 나중에 추가하던지 하자
  ...
  async findUsersByOptions(options: FindOneOptions<User>): Promise<ServiceData<User>> {
    try {
      ...
      throw new InternalServerError({ ...error, name: 'FIND_USERS_ERROR' });
    }
  }
  ...
  // # 팔로우 관계 체크
  async checkFollow(user: User, targetUser: User): Promise<ServiceData<boolean>> {
    try {
      const isFollowing = !!(await Follow.findOneById(user.id, targetUser.id));
      return successData(isFollowing);
    } catch (error) {
